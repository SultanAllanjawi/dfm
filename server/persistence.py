...
Flat-JSON storage; survives server restarts.
"""
...
def update_signal_status(ticker: str, signal: str, entry: float,
    ...
    if live_price and _tp and _sl and _status == "ACTIVE":
        if _sig == "BUY":
            if live_price >= _tp:
                _status = "HIT_TP"; sig_data["status"] = "HIT_TP"; save_active_signals(signals)
            elif live_price <= _sl:
                _status = "HIT_SL"; sig_data["status"] = "HIT_SL"; save_active_signals(signals)
        elif _sig == "SELL":
            if live_price <= _tp:
                _status = "HIT_TP"; sig_data["status"] = "HIT_TP"; save_active_signals(signals)
            elif live_price >= _sl:
...
# ── Rolling accuracy history (one real entry per fresh retrain) ──
ACCURACY_HISTORY_FILE = os.path.join(DATA_DIR, "accuracy_history.json")

def record_accuracy(ticker: str, filtered_accuracy: float, raw_accuracy: float):
    """Append one real data point whenever a ticker's signal is freshly
    retrained (cache miss) — not on every request, since cached responses
    reuse the same model and would just duplicate the same number."""
    try:
        history = {}
        if os.path.exists(ACCURACY_HISTORY_FILE):
            with open(ACCURACY_HISTORY_FILE) as f:
                history = json.load(f)
        entries = history.get(ticker, [])
        entries.append({
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "filteredAccuracy": round(filtered_accuracy, 4),
            "accuracy": round(raw_accuracy, 4),
        })
        history[ticker] = entries[-50:]
        with open(ACCURACY_HISTORY_FILE, "w") as f:
            json.dump(history, f, indent=2)
    except Exception:
        pass

def load_accuracy_history(ticker: str) -> list:
    try:
        if os.path.exists(ACCURACY_HISTORY_FILE):
            with open(ACCURACY_HISTORY_FILE) as f:
